 ...
 *
 * 用法：
 *   node send-digest.js            # 前台运行
 *   node send-digest.js --now      # 立即发送一次
 *   node send-digest.js --send-only # 只发送邮件
 */
...
const SAAS_ROOT = 'D:/project/devpulse-saas';
let prisma = null;
try {
...
const BRAND = {
  ...
  secondary: '#7C3AED',   // 辅色 purple
  accent: '#F59E0B',      // 强调色 amber
  bg: '#F0F2F8',          // 邮件外背景
  sectionBg: '#F7F8FC',   // 区块背景
  aiBg: '#F5F3FF',        // AI分析背景
  textPrimary: '#1E1B4B', // 主文字
  textSecondary: '#6B7280', // 副文字
  textMuted: '#9CA3AF',   // 辅文字
  border: '#E5E7EB',      // 边框
  divider: '#E0E2EC',     // 分割线
...
// 版块定义：分类 → 版块映射（同时覆盖 raw 和 enhanced 两种分类名）
const ZONES = [
...
// 来源标签颜色
const SOURCE_COLORS = {
  '微博': '#ECFDF5', '知乎': '#F3E8FF', 'B站': '#FFF7ED',
  '36氪': '#EFF6FF', '抖音': '#FDF2F8', 'HackerNews': '#F3E8FF',
  'DEV.to': '#ECFDF5', 'Product Hunt': '#F0F9FF', 'V2EX': '#F3E8FF',
  '贴吧': '#FFF7ED', '虎扑': '#FEF3C7', '豆瓣电影': '#ECFDF5',
};
            ...
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="left">
              <tr>
                <td style="vertical-align:middle;">
                  <div style="width:36px;height:36px;background-color:${BRAND.primary};border-radius:8px;text-align:center;line-height:36px;font-size:16px;color:#FFFFFF;font-weight:700;font-family:${BRAND.fontEn};">D</div>
                </td>
                <td style="padding-left:12px;vertical-align:middle;">
                  <span style="font-family:${BRAND.fontEn};font-size:20px;font-weight:700;color:${BRAND.textPrimary};">DevPulse AI</span>
                </td>
              </tr>
            </table>
...
function top5Section(items) {
  ...
  <tr>
    <td style="padding:10px 28px 0 28px;">
      <div style="border-top:1px dashed ${BRAND.divider};font-size:1px;line-height:1px;">&nbsp;</div>
    </td>
  </tr>`;
    }
    itemsHtml += `
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
        <tr>
          <td width="32" style="vertical-align:top;padding-top:2px;">
            <div style="width:24px;height:24px;background-color:${BRAND.primary};border-radius:50%;text-align:center;line-height:24px;font-size:13px;font-weight:700;color:#FFFFFF;font-family:${BRAND.fontEn};">${idx + 1}</div>
          </td>
          <td style="vertical-align:top;padding-left:12px;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
              <tr>
                <td style="padding-bottom:4px;">
                  <a href="${item.url}" style="font-family:${BRAND.font};font-size:15px;font-weight:600;color:${BRAND.textPrimary};text-decoration:none;line-height:22px;">${esc(item.title)}</a>
                  ${catTag(item.category)}${scoreBadge(item.value)}
                </td>
              </tr>
            </table>
  ...
  return `
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
       style="background-color:${BRAND.sectionBg};border-radius:12px;overflow:hidden;margin:24px 0 32px 0;">
  <tr>
    <td style="padding:24px 28px 0 28px;">
      ${sectionTitle('今日必读 TOP 5', '30秒抓住今天最重要的事')}
    </td>
  </tr>
  <tr>
  ...
  ${itemsHtml}
  <tr>
    <td style="padding:20px 28px;font-size:1px;line-height:1px;">&nbsp;</td>
  </tr>
</table>`;
}

/** 生成 section 标题行：标签指示器 + section 标题 */
function sectionHeader(activeTab, tabLabel, sectionLabel, count) {
  ...
  for (const t of tabs) {
    ...
    const bg = isActive ? BRAND.primary : '#F3F4F6';
    const color = isActive ? '#FFFFFF' : BRAND.textSecondary;
    const fw = isActive ? '600' : '500';
    tabsHtml += `<td style="padding:5px 16px;background-color:${bg};border-radius:20px;font-size:12px;font-weight:${fw};color:${color};font-family:${BRAND.font};">${t.label}</td>
<td width="6" style="font-size:1px;">&nbsp;</td>`;
  }
...
function generateGithubCardsHtml(repos, summaries, analysis) {
  ...
  for (const sec of sections) {
    ...
    for (const repo of items) {
      ...
      html += `
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
       style="background-color:#FFFFFF;border:1px solid ${BRAND.border};border-radius:12px;overflow:hidden;margin:0 0 16px 0;">
  <tr>
    <td style="padding:0;">
            ...
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
              <tr>
                ...
                <td width="6" style="vertical-align:top;">
                  <div style="width:4px;min-height:40px;border-radius:2px;background-color:${langColor};"></div>
                </td>
                <!-- 项目信息 -->
            ...
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
              <tr>
                ...
                <td width="260" style="vertical-align:top;padding-right:16px;">
                  <a href="${repo.url}" style="text-decoration:none;">
                    <!--[if mso]>
                    <v:rect xmlns:v="urn:schemas-microsoft-com:vml" fill="true" stroke="false" style="width:260px;height:146px;">
                      <v:fill type="frame" src="${ogImageUrl}" color="#F3F4F6"/>
                      <v:textbox inset="0,0,0,0"><div style="text-align:center;padding-top:56px;font-size:13px;color:#9CA3AF;">${esc(repo.name)}</div></v:textbox>
                    </v:rect>
                    <![endif]-->
                    <img src="${ogImageUrl}" width="260" style="display:block;width:260px;height:146px;border-radius:8px;border:1px solid ${BRAND.border};background-color:#F3F4F6;font-size:13px;color:#9CA3AF;" alt="${esc(repo.name)} 预览" />
                  </a>
                </td>
                <!-- 右侧：AI深度分析（紧凑两列布局） -->
                <td style="vertical-align:top;">
                  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
                         style="background-color:${BRAND.aiBg};border-radius:8px;border-left:3px solid ${BRAND.secondary};">
                    <tr>
                      <td style="padding:10px 14px 4px 14px;" colspan="2">
                        <span style="font-family:${BRAND.font};font-size:12px;font-weight:700;color:${BRAND.secondary};letter-spacing:1px;">&#x2728; AI 分析</span>
                      </td>
                    </tr>
                    <tr>
                      <td width="50%" style="padding:4px 14px 4px 14px;vertical-align:top;">
                        <span style="font-size:10px;color:${BRAND.textMuted};font-weight:600;">&#x1F3AF; 用途</span><br>
                        <span style="font-size:12px;color:#374151;font-family:${BRAND.font};line-height:1.5;">${esc(deep.purpose)}</span>
                      </td>
                      <td width="50%" style="padding:4px 14px 4px 0;vertical-align:top;">
                        <span style="font-size:10px;color:${BRAND.textMuted};font-weight:600;">&#x1F527; 解决</span><br>
                        <span style="font-size:12px;color:#374151;font-family:${BRAND.font};line-height:1.5;">${esc(deep.problem)}</span>
                      </td>
                    </tr>
                    <tr>
                      <td style="padding:0 14px 4px 14px;vertical-align:top;">
                        <span style="font-size:10px;color:${BRAND.textMuted};font-weight:600;">&#x1F4CD; 领域</span><br>
                        <span style="display:inline-block;margin-top:1px;padding:1px 8px;background-color:${BRAND.secondary}18;border-radius:10px;font-size:11px;color:${BRAND.secondary};font-weight:500;">${esc(deep.domain)}</span>
                      </td>
                      <td style="padding:0 14px 4px 0;vertical-align:top;">
                        <span style="font-size:10px;color:${BRAND.textMuted};font-weight:600;">&#x1F525; 上榜</span><br>
                        <span style="font-size:12px;color:#374151;font-family:${BRAND.font};line-height:1.5;">${esc(deep.reason)}</span>
                      </td>
                    </tr>
                    ${deep.highlight ? `<tr>
                      <td colspan="2" style="padding:4px 14px 10px 14px;background-color:${BRAND.primary}12;border-radius:0 0 7px 0;">
                        <span style="font-size:10px;color:${BRAND.textMuted};font-weight:600;">&#x2B50; 亮点</span><br>
                        <span style="font-size:12px;color:${BRAND.primary};font-family:${BRAND.font};line-height:1.5;font-weight:500;">${esc(deep.highlight)}</span>
                      </td>
                    </tr>` : ''}
                  </table>
                </td>
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
        <tr>
          <td style="padding:0 24px;">
            <div style="${borderTop}padding:10px 0;background:#F9FAFB;font-size:13px;color:#374151;line-height:1.6;font-family:${BRAND.font};">
              ${esc(ai.summary)}
              <span style="display:inline-block;float:right;color:${scoreColor};font-weight:600;font-size:12px;">${aiLabel} ${ai.value}/30</span>
            </div>
          </td>
        </tr>
      </table>`;
...
function hotNewsSection(digestItems) {
  ...
  // 将条目按分类归组
  const catMap = {}; // { categoryName: [item, ...] }
  let currentCat = '';
  ...
  for (const zone of ZONES) {
    ...
    for (const cat of Object.keys(catMap)) {
      if (!cat) continue; // 跳过空分类名
      const matched = zone.cats.some(zc => cat.includes(zc) || zc.includes(cat));
      if (matched && catMap[cat].length > 0) {
        zoneCats.push({ name: cat, items: catMap[cat] });
        delete catMap[cat]; // 标记已处理
      }
    }
    ...
    html += `
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
       style="background-color:${zone.bg};border:1px solid ${zone.color}22;border-radius:12px;overflow:hidden;margin:0 0 16px 0;">
  <tr>
    <td style="padding:0;">
      <!-- 版块标题栏 -->
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
    ...
    for (const cat of zoneCats) {
            ...
            <table role="presentation" cellpadding="0" cellspacing="0" border="0">
              <tr>
                <td width="8" style="vertical-align:middle;">
                  <table role="presentation" cellpadding="0" cellspacing="0" border="0">
                    <tr><td width="6" height="6" style="background-color:${catDotColor};border-radius:50%;"></td></tr>
                  </table>
                </td>
                <td style="padding-left:6px;vertical-align:middle;">
                  <span style="font-size:12px;font-weight:600;color:${zone.color};font-family:${BRAND.font};opacity:0.8;">${esc(cat.name)}</span>
                </td>
              </tr>
            </table>
          </td>
      ...
      for (const item of cat.items) {
        const scoreBadge = item.value >= 27
          ? `<span style="display:inline-block;margin-left:6px;padding:1px 8px;background-color:#FEF3C7;border-radius:4px;font-size:11px;color:#B45309;font-weight:600;font-family:${BRAND.font};">必读 ${item.value}/30</span>`
          : item.value >= 21
            ? `<span style="display:inline-block;margin-left:6px;padding:1px 8px;background-color:#E0E7FF;border-radius:4px;font-size:11px;color:#4338CA;font-weight:600;font-family:${BRAND.font};">推荐 ${item.value}/30</span>`
            : '';
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
        <tr>
          <td style="padding:6px 20px 6px 20px;border-top:1px solid ${zone.color}11;">
            <a href="${item.url}" style="font-size:13px;font-weight:500;color:#374151;text-decoration:none;line-height:20px;font-family:${BRAND.font};">${esc(item.title)}</a>
            ${scoreBadge}
    ...
    html += `
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
        <tr><td style="padding:8px 20px;font-size:1px;line-height:1px;">&nbsp;</td></tr>
      </table>
    </td>
  </tr>
</table>`;
...
<div style="padding:32px 24px 16px 24px;">
  <div style="border-top:1px solid ${BRAND.border};font-size:1px;line-height:1px;">&nbsp;</div>
</div>
<div style="padding:16px 24px 32px 24px;text-align:center;">
  <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center">
    <tr>
      <td style="font-size:12px;color:${BRAND.textMuted};line-height:20px;font-family:${BRAND.font};">
        DevPulse AI &nbsp;|&nbsp;
        <a href="#" style="color:${BRAND.textSecondary};text-decoration:underline;">GitHub</a> &nbsp;|&nbsp;
        <a href="#" style="color:${BRAND.textSecondary};text-decoration:underline;">反馈建议</a>
      </td>
    </tr>
    <tr>
      <td style="padding-top:8px;font-size:11px;color:${BRAND.textMuted};font-family:${BRAND.font};">
        &copy; 2026 DevPulse AI. Powered by Kimi + Node.js.
      </td>
    </tr>
  </table>
</div>`;
...
function runCollect(cmd, label) {
  console.log(`  采集 ${label} ...`);
  try {
    execSync(cmd, { cwd: scriptsDir, stdio: 'pipe', timeout: 180000 });
    console.log(`    ✓ ${label} 采集完成`);
    return true;
  } catch (e) {
    console.log(`    ✗ ${label} 采集失败: ${e.message?.slice(0, 100)}`);
    return false;
  }
}
...
function runPublish() {
  console.log('  GitHub Pages 发布 ...');
  try {
    execSync('node publish-pages.js', { cwd: scriptsDir, stdio: 'pipe', timeout: 60000 });
    console.log('    ✓ Pages 发布完成');
    return true;
  } catch (e) {
    console.log(`    ✗ Pages 发布失败: ${e.message?.slice(0, 100)}`);
    return false;
  }
}
...
async function sendEmail() {
  ...
  if (deepEnhancedMd) {
    ...
    if (deepItems.length > 0) {
      body += hotNewsSection(deepItems);
    }
  }

  // [E] 热点资讯
  if (digestItems.length > 0) {
    body += hotNewsSection(digestItems);
  }
...
${body}
  ...
  // 获取收件人列表：优先从数据库读取活跃订阅者，fallback 到 .env
  ...
  try {
    await transporter.sendMail({
      ...
      to: recipients,
      subject: `DevPulse AI | ${today} ${weekday}`,
      html: htmlBody,
      attachments,
    });
    console.log('  ✓ 邮件发送成功');
...
function runPipeline(today) {
  runCollect('node daily-digest.js', '热点日报');
  runCollect('node github-trending.js', 'GitHub Trending');
  runCollect('python scrapling-deep.py', '海外深度抓取');
...
async function runDaemon() {
  console.log('========================================');
  console.log('  DevPulse AI 守护进程已启动');
  console.log('  每天 07:00 自动采集，09:00 发送邮件');
  ...
  function check() {
    ...
    // 07:00-07:05 采集数据
    if (now.getHours() === 7 && now.getMinutes() <= 5 && collectedToday !== today) {
      console.log(`\n[${now.toLocaleTimeString()}] 开始采集 ...\n`);
      runPipeline(today);
...
if (sendNow || sendOnly) { runOnce(); } else { runDaemon(); }
